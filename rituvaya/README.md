...
- **Onboarding** (8 guided steps, resumable): language, name, time/units/theme, wake-up/bedtime/optional meals, hydration goal, first item, its schedule, routine preview, notification permission with explanation.
...
- **Items**: display name, generic name, brand, ingredients, strength + unit, serving size, dose amount + unit, form (capsule, tablet, powder, liquid, gummy, drops, injection, custom), purpose, notes. Small generic seed catalog for names and forms only (no dosing guidance), manual entry always available.
