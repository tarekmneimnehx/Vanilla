...
import { hasOptionalDetails, withForm, type DraftErrors, type ItemDraft } from './itemDraft';
...
/**
 * A name is all this asks for. Everything else — brand, strength, form, dose,
 * notes — is optional and stays folded away, because plenty of people only want
 * a list of what they take. An item that already carries those details opens
 * with them showing, so editing never buries anything.
 */
export function ItemForm({ draft, onChange, errors }: ItemFormProps) {
  ...
  const [showBrand, setShowBrand] = useState(() => draft.brand.trim().length > 0);
  const [showDetails, setShowDetails] = useState(() => hasOptionalDetails(draft));
  ...
  return (
    <View style={{ gap: theme.spacing.md }}>
      ...
      <Card style={{ gap: theme.spacing.md }}>
        ...
        />
        <Field label={t('item.name')} placeholder={t('item.namePlaceholder')} value={draft.displayName} onChangeText={(v) => set('displayName', v)} error={errors.displayName ? t(errors.displayName) : null} autoCapitalize="words" ltr autoFocus={!draft.displayName} />
        {showBrand ? (
          <Field label={t('item.brand')} placeholder={t('item.brandPlaceholder')} value={draft.brand} onChangeText={(v) => set('brand', v)} autoCapitalize="words" ltr autoFocus />
        ) : (
          <View style={{ alignItems: 'flex-start' }}>
            <Button label={t('item.addBrand')} variant="ghost" icon="plus" onPress={() => setShowBrand(true)} />
          </View>
        )}
      </Card>
      ...
      <View style={{ alignItems: 'flex-start' }}>
        <Button
          label={showDetails ? t('item.fewerDetails') : t('item.moreDetails')}
          variant="ghost"
          iconEnd={showDetails ? 'chevron-up' : 'chevron-down'}
