...
async function main() {
  ...
  await click('Next');
  await shot('03-onboarding-preferences');
  await click('Next');
  await shot('04-onboarding-day');
  await click('Set Breakfast');
  await click('Next');
  await page.getByPlaceholder('Amount').fill('2500');
  await shot('05-onboarding-hydration');
  await click('Next');
  await page.getByPlaceholder('e.g. Vitamin D, Magnesium').fill('vit d');
  await shot('06-onboarding-catalog');
  await click('Vitamin D3');
  await shot('07-onboarding-item-details');
  await click('Next');
  await shot('08-onboarding-schedule');
  await click('Save');
  await page.waitForSelector('text=Your routine preview', { timeout: 15000 });
  await shot('09-onboarding-preview');
  await click('Next');
  await shot('10-onboarding-notifications');
